  ...
  return (
    <>
      <div className="flex flex-col md:flex-row gap-4 md:gap-0 justify-start w-full">
        ...
        <div className="flex flex-row md:justify-end justify-start w-full">
          <CustomButton
            variant="text"
            classname={
              def
                ? `text-white hover:text-white bg-primary-main hover:bg-primary-main border-2 placeholder   md:w-[168px] w-full md:text-base text-xs`
                : 'text-black hover:text-white border-2  bg-[#F7F7F7] hover:bg-primary-main  md:w-[168px] w-full  md:text-base text-xs'
            }
            onClick={option}
          >
            2021-2022 он
          ...
          <CustomButton
            ...
            classname={
              def1
                ? `text-white hover:text-white bg-primary-main hover:bg-primary-main border-2  md:w-[168px] w-full  md:text-base text-xs`
                : 'text-black hover:text-white border-2  bg-[#F7F7F7] hover:bg-primary-main  md:w-[168px] w-full  md:text-base text-xs'
            }
            onClick={option1}
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-none md:grid-rows-2 md:grid-flow-col gap-6 justify-start my-[36px]">
        {data.map((x: any, i: number) => {
          return (
            <div key={i}>
              <div
                className="flex flex-col justify-end w-full bg-gradient-to-t from-gray-700 to-gray-0 hover:cursor-pointer"
                onMouseEnter={() => setToggle(i)}
                onMouseLeave={() => setToggle(null)}
              >
                <Image src={x.image.url} alt="" width={184} height={256} className="hover:mix-blend-overlay" />
                <div
                  className={`flex flex-col absolute ml-4 pointer-events-none ${toggle == i ? 'visible' : 'hidden'}`}
                >
                  <div className="flex flex-col bg-primary-main justify-center text-center text-white w-28 h-8 mb-1">
                    <Typo variant="body-semibold" className="text-sm">
                      {toUpper(x.type)}
                    </Typo>
                  </div>
